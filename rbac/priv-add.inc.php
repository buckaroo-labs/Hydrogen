...
?>
    <form action="admin.php?p=Privs" method="post">
    <!-- these would be for an edit
    <input type="hidden" id="id" name="id" value="<?php echo $personID; ?>">
    <input type="hidden" id="column" name="column" value="<?php echo $column; ?>">
    -->
    <h3>New privilege</h3>
    <input type="hidden" id="flow" name="flow" value="insert">
    Name (alphanumeric/underscore): <input type="text" id="privname" name="privname" pattern="<?php echo $inputPattern; ?>" 
        size="20" maxlength="<?php echo $maxLength ?>" value="<?php echo $oldValue; ?>"><br>
    Description (alphanumeric/space): <input type="text" id="pdescription" name="pdescription" pattern="<?php echo $inputPattern2; ?>" 
